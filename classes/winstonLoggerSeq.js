...
function onMessage (m) {
    if (m.type == "config") {
        ...
        m.data.message = m.data.message.trim();
        ...
        else if (m.data.level == "warn") logger.warn("", {replacementData: m.data});
    }
}

process.on('SIGINT', () => {}); //Prevent the process from closing
process.on('SIGTERM', () => {}); //Prevent the process from closing
process.on("SIGUSR1", function() {}); //Prevent the process from closing
